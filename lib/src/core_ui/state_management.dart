...
extension CommonProviders on BuildContext {
  FocusNode get focusNode => read();
  TextEditingController get textEditingController => read();
  T selectFocusNode<T>(T Function(FocusNode n) fn) => select(fn);
  T selectTextEditingController<T>(T Function(TextEditingController c) fn) =>
...
abstract class MultiProviderWidget extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final providers = [...this.providers, ...getProviders(context)];
    if (providers.isEmpty) return buildChild(context);
...
class ValueProvider<T extends ChangeNotifier> extends ListenableProvider<T> {
  ...
  ValueProvider.value({
    ...
    void Function(T? notifier)? onNotified,
    UpdateShouldNotify<T>? updateShouldNotify,
    super.builder,
    super.child,
  }) : super.value(
         updateShouldNotify: (prev, val) {
           onNotified?.call(val);
           return updateShouldNotify?.call(prev, val) ?? true;
         },
       );
}
